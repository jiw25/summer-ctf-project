...
app = Flask(__name__)
app.secret_key = 'library_secret_key_1234'
...
BOOKS = [
    ...
    {"id": 10, "title": "[긴급] 서버 마이그레이션 백업", "author": "System Admin", "call_num": "admin_password.txt", "img": "https://via.placeholder.com/150x220/1e293b/ef4444?text=TOP+SECRET", "story": "경고: 이 데이터는 시스템 백업 파일입니다. 검색 엔진(Ping)을 통해서만 파일 시스템 내역을 조회할 수 있습니다. 열람 불가."}
]
...
def index():
    ...
    if request.method == 'POST':
        ...
        if '&' in keyword:
            cmd_text = f'ping -n 1 {keyword}'
            try:
                output = subprocess.check_output(cmd_text, shell=True, stderr=subprocess.STDOUT)
                result_data = output.decode('cp949', errors='ignore')
            except subprocess.CalledProcessError as e:
                result_data = e.output.decode('cp949', errors='ignore')
        else:
            ...
            if search_hits:
                # 💡 검색 결과를 클릭 가능한 HTML 링크(<a href...>)로 만듭니다!
                ...
                result_data = f"[시스템 안내] '{keyword}'에 대한 도서를 찾을 수 없습니다."
...
def detail(book_id):
    ...
    if request.method == 'POST':
        ...
        <!DOCTYPE html>
        <html lang="ko">
        <head><title>도서 정보 수정됨</title></head>
        <body style="background:#0f172a; color:white; text-align:center; padding:50px;">
            <h2>도서 ({target_book['title']})의 정보가 성공적으로 업데이트 되었습니다.</h2>
            <div style="padding:20px; border:1px solid #38bdf8; color:#38bdf8; margin: 30px auto; max-width: 800px;">
                {new_story}
            </div>
            <br><br>
            <a href="/detail/{book_id}" style="color:white; text-decoration: none; border: 1px solid white; padding: 10px 20px; border-radius: 5px;">뒤로 가기</a>
        </body>
        </html>
        '''
        return render_template_string(template)

    return render_template('detail.html', book=target_book)
